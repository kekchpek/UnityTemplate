...
{
    internal class SaveFileCodecV1 : ISaveFileCodec, ISaveCodecAdapter, ILoadCodecAdapter
    {

        private readonly ICustomCodecsProvider _customCodecsProvider;
        private readonly int _currentCustomCodecsVersion;

        public SaveFileCodecV1(
            ICustomCodecsProvider customCodecsProvider, int currentCustomCodecsVersion)
        {
        ...
        {
            var codecVersion = ReadStruct<int>(inputStream);
            var metaDataLength = ReadStruct<short>(inputStream);
            var initialPosition = inputStream.Position;
            foreach (var (key, val) in DecodeInternal(inputStream, codecVersion))
            {
        ...
        public IEnumerable<(string key, ILoadStream val)> DecodeMeta(Stream inputStream)
        {
            ...
            foreach (var valueTuple in DecodeInternal(metaStream, codecVersion))
            {
                yield return valueTuple;
            }
        }

        private IEnumerable<(string key, ILoadStream val)> DecodeInternal(Stream inputStream, int? customCodecVersion)
        {
            while (true)
            {
                if (TryReadKey(inputStream, out var key))
                {
                    var value = ReadValue(inputStream, customCodecVersion);
        ...
        {
            WriteStruct(outputStream, _currentCustomCodecsVersion);
            ...
            foreach (var saveData in data)
            {
                    ...
                    switch (value)
                    {
                        case List<ISaveObject> saveObjects:
                            for (var i = 0; i < saveObjects.Count; i++)
                            {
                                ...
                                saveObject.Serialize(saveStream, _currentCustomCodecsVersion);
                                saveStream.Dispose();
                ...
                else 
                {
                    if (saveData.Data is IEnumerable enumerable)
                    {
                        ...
                        {   
                            WriteString(outputStream, saveData.DataNames[i++]);
                            WriteCustom(outputStream, o, customCodec, _currentCustomCodecsVersion);
                        }
                    }
                    else 
                    {
